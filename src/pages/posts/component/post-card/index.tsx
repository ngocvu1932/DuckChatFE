...
const PostCard: React.FC<IPostCardProps> = ({post, onToggleLike, onAddComment}) => {
  ...
  return (
    <article className="rounded-2xl border border-white/80 bg-white/95 p-4 shadow-sm shadow-slate-200/70 transition duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-slate-200/80 sm:p-5">
        ...
        <div className={`mt-4 grid gap-3 ${post.images.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
          {post.images.map((image) => {
            return (
              <div
                key={image}
                className="group overflow-hidden rounded-2xl bg-slate-100 shadow-sm ring-1 ring-slate-200"
              >
                <img
                  src={image}
                  alt="post"
                  className={`${post.images.length === 1 ? 'h-72' : 'h-44 sm:h-52'} w-full object-cover transition duration-500 group-hover:scale-105`}
                />
              </div>
            );
          })}
      ...
      <div className="mt-4 flex items-center gap-3 border-b border-slate-100 pb-4 text-sm font-semibold text-slate-500">
        <span className="rounded-full bg-rose-50 px-3 py-1 text-rose-500">{post.likeCount} lượt thích</span>
        <span className="rounded-full bg-sky-50 px-3 py-1 text-sky-600">{post.commentCount} bình luận</span>
      </div>

      <div className="mt-4 flex flex-wrap items-center gap-3">
        <button
          type="button"
          className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-sm font-bold shadow-sm transition duration-200 focus:outline-none focus:ring-4 active:scale-95 ${
            post.isLiked
              ? 'bg-rose-50 text-rose-500 ring-1 ring-rose-100 hover:bg-rose-100 focus:ring-rose-100'
        ...
        <button
          type="button"
          className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-sm font-bold shadow-sm transition duration-200 focus:outline-none focus:ring-4 focus:ring-sky-100 active:scale-95 ${
            isCommentOpen
      ...
      {isCommentOpen && (
        <div className="mt-4 space-y-3 rounded-2xl bg-slate-50/60 p-3 ring-1 ring-slate-100">
          ...
          {post.comments.length > 0 ? (
            ...
            <div className="rounded-2xl bg-white p-4 text-sm font-medium text-slate-500 ring-1 ring-slate-100">
